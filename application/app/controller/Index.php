...
class Index extends Controller
{
    ...
    public function m()
    {
        ...
        $sid = input('uid');
        $num = db('user')->where('id', $uid)->count();
        $config = db("config")->where("type", "app_download_url")->find();
        if ($config) {
            $app_download_url = $config["content"];
        }
        if ($num > 0) {
            $ip = $this->getIP();
            $ipnum = db('share')->where('ip', $ip)->count();
        ...
        return view('m', [
            'share' => $share_ma, 'sid' => $sid,'app_download_url' => $app_download_url]);
    }
    ...
    public function kefulingjiang(){
        return view('kefulingjiang', []);
    }
    public function videoiframe()
    {
        return view("videoiframe",[
            'url'=>input('url')]
    );
    }
    ...
    public function getvip(){
        return json(['content'=>"    请联系客服微信：juzikefu168 获取，备注：获取橘子会员"]);
    }
